...
# Каждый элемент связывает 4 узла (квадратные элементы)
	...
	# *******2.5 Работа с файлами (сохранить данные и считать) ****

	using DelimitedFiles

# Функция сохранения в текстовый файл
function save_mesh_and_params_txt(filename, nodes, connectivity, params)
    open(filename, "w") do io
        # Записываем количество строк в nodes и connectivity
        println(io, "# NODES")
        writedlm(io, nodes, ' ')
        
        println(io, "\n# CONNECTIVITY")
        writedlm(io, connectivity, ' ')
    ...
    println("Сетка и параметры сохранены в $filename")
end

# Функция загрузки из текстового файла
function load_mesh_and_params_txt(filename)
    ...
    for line in lines
        if startswith(line, "# NODES")
            ...
            param_section = false
            continue
        elseif startswith(line, "# CONNECTIVITY")
            node_section = false
        ...
        if node_section
            ...
            push!(nodes, row)
        elseif connect_section
            row = parse.(Int, split(line))
            push!(connectivity, row)
        elseif param_section
            key_val = split(line, '=')
            if length(key_val) == 2
                key = Symbol(strip(key_val[1]))
                val = tryparse(Float64, strip(key_val[2]))
                if val === nothing
                    val = strip(key_val[2])  # если не число, оставляем как строку
                end
                params[key] = val
            end
        end
    ...
    nodes = reduce(hcat, nodes)'
    connectivity = reduce(hcat, connectivity)'

    return nodes, connectivity, params
end
...
# Сохраняем всё в txt
save_mesh_and_params_txt("mesh_and_params.txt", nodes, connectivity, params)

# Загружаем обратно
nodes_loaded, connectivity_loaded, params_loaded = load_mesh_and_params_txt("mesh_and_params.txt")

println("Загруженные параметры: ", params_loaded)
...
# Закрепляем левую грань (x=0)
for j in 0:ny
    node = j * (nx + 1) + 1  # в Julia индексация с 1
    push!(fixed_dofs, 2 * (node - 1) + 1)  # x-перемещение (нумерация с 1)
    push!(fixed_dofs, 2 * (node - 1) + 2)  # y-перемещение
end

# ***** 8. ПРИЛОЖЕНИЕ НАГРУЗКИ СВЕРХУ *****
# Инициализируем вектор внешних сил (по 2 степени свободы на узел)
F = zeros(2 * num_nodes)

# Расчет длины элемента по оси x
element_length_x = Lx / nx

# Сила на узел: давление * длина элемента * толщина
node_force = pressure * element_length_x * thickness

# На крайние узлы (левый и правый) приходится половина силы
end_node_force = node_force / 2

# Применение нагрузки к верхней границе (y = Ly)
for i in 0:nx
    node = ny * (nx + 1) + i + 1  # индексация с 1
    dof_y = 2 * node  # y-компонента силы
    if i == 0 || i == nx
        # Крайние узлы — половина силы
        F[dof_y] = -end_node_force  # "-" для направления вниз
    else
        # Внутренние узлы — полная сила
        F[dof_y] = -node_force
    end
end
...
exact_solution(x, y) = [0.0, pressure, 0.0]
...
function calculate_sigma_xx_error_L2(numerical_stresses, connectivity, nodes, exact_sigma_xx; integration_order=2)
    ...
    for e in axes(connectivity, 1)
        ...
        num_sigma_xx = numerical_stresses[e][2]
...
function plot_combined_field(element_stresses, connectivity, nodes, U; 
                              ...
                              deformed=true)    # Рисовать ли на деформированной сетке
    ...
    plt = plot(xlabel="x (мм)", ylabel="y (мм)",
...
plot_combined_field(element_stresses, connectivity, nodes, U, 
                    filename="C:\\Users\\Admin\\Music\\RESULTS\\UP\\sigma_xx_combined1.png", 
                    plot_type=1)

# Отрисовка σyy
plot_combined_field(element_stresses, connectivity, nodes, U, 
                    filename="C:\\Users\\Admin\\Music\\RESULTS\\UP\\sigma_yy_combined1.png", 
                    plot_type=2)

# Отрисовка τxy
plot_combined_field(element_stresses, connectivity, nodes, U, 
                    filename="C:\\Users\\Admin\\Music\\RESULTS\\UP\\tau_xy_combined1.png", 
                    plot_type=3)

# Отрисовка модуля перемещений
plot_combined_field(element_stresses, connectivity, nodes, U, 
                    filename="C:\\Users\\Admin\\Music\\RESULTS\\UP\\displacement_combined1.png", 
                    plot_type=9)

	# Отрисовка u_x
plot_combined_field(element_stresses, connectivity, nodes, U,
                    filename="C:\\Users\\Admin\\Music\\RESULTS\\UP\\u_x1.png",
                    plot_type=7)

# Отрисовка u_y
plot_combined_field(element_stresses, connectivity, nodes, U,
                    filename="C:\\Users\\Admin\\Music\\RESULTS\\UP\\u_y1.png",
                    plot_type=8)
# Деформации
plot_combined_field(element_stresses, connectivity, nodes, U,
                    filename="C:\\Users\\Admin\\Music\\RESULTS\\UP\\strain_exx1.png",
                    plot_type=4)

plot_combined_field(element_stresses, connectivity, nodes, U,
                    filename="C:\\Users\\Admin\\Music\\RESULTS\\UP\\strain_eyy1.png",
                    plot_type=5)

plot_combined_field(element_stresses, connectivity, nodes, U,
                    filename="C:\\Users\\Admin\\Music\\RESULTS\\UP\\strain_gxy1.png",
                    plot_type=6)
...
# Отрисовка энергии
plot_combined_field(element_stresses, connectivity, nodes, U;
                    filename="C:\\Users\\Admin\\Music\\RESULTS\\UP\\energy_distribution1.png",
                    plot_type=10)
	# Отрисовка напряжения Мизеса
plot_combined_field(element_stresses, connectivity, nodes, U,
                    filename="C:\\Users\\Admin\\Music\\RESULTS\\UP\\von_mises1.png",
                    plot_type=11)
